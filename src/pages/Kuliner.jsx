...
const Kuliner = () => {
  ...
  const kulinerImages = {
    1: "https://images.unsplash.com/photo-1548943487-a2e4d43b4850?q=80&w=800&auto=format&fit=crop", // Soto Karawang
    2: "https://images.unsplash.com/photo-1582238479260-264627d781b4?q=80&w=800&auto=format&fit=crop", // Pepes Ikan Mas
    3: "https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?q=80&w=800&auto=format&fit=crop", // Karedok
    4: "https://images.unsplash.com/photo-1626132647523-66f5bf380027?q=80&w=800&auto=format&fit=crop", // Gemblong
    5: "https://images.unsplash.com/photo-1505253716362-afaea1d3d1af?q=80&w=800&auto=format&fit=crop", // Dodol Karawang
    6: "https://images.unsplash.com/photo-1553909489-cd47e0907980?q=80&w=800&auto=format&fit=crop"  // Es Cendol
  };
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <div className="bg-cokelat rounded-[40px] overflow-hidden shadow-2xl flex flex-col md:flex-row">
            <motion.div 
              ...
              className="md:w-1/2 p-6 sm:p-10 lg:p-16 flex flex-col justify-center text-krem"
            >
              <div className="flex items-center gap-3 mb-6 text-emas">
                <Flame size={24} />
                <span className="font-bold tracking-widest uppercase text-xs md:text-sm">Resep Legendaris</span>
              </div>
              <h2 className="font-playfair text-3xl sm:text-4xl lg:text-6xl font-bold text-white mb-6">Soto Karawang</h2>
              <p className="text-krem/80 text-sm md:text-lg leading-relaxed mb-8">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
                  <h4 className="font-bold text-emas mb-1 text-sm md:text-base">Rasa</h4>
                  <p className="text-xs md:text-sm">Gurih, Rempah Kuat</p>
                </div>
                <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
                  <h4 className="font-bold text-emas mb-1 text-sm md:text-base">Pendamping</h4>
                  <p className="text-xs md:text-sm">Nasi Hangat, Emping</p>
                </div>
              </div>
            </motion.div>
            <motion.div 
              initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
              className="md:w-1/2 relative min-h-[250px] md:min-h-full"
            >
              <img 
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-4">
              {categories.map(cat => (
                <button
                  ...
                  className={`px-6 py-2.5 sm:px-8 sm:py-3 rounded-2xl font-bold text-sm sm:text-base transition-all ${
                    filter === cat 
            ...
            <AnimatePresence>
              {filteredData.map((item, index) => (
                <motion.div
                  ...
                  key={item.id}
                  className="bg-white rounded-[32px] overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.06)] border border-emas/10 group hover:-translate-y-2 transition-transform flex flex-col h-full"
                >
                  <div className="h-56 sm:h-64 relative overflow-hidden">
                    <img 
                      src={kulinerImages[item.id] || "https://images.unsplash.com/photo-1604908176997-125f25cc6f3d?q=80&w=600&auto=format&fit=crop"}
                      alt={item.nama}
                    ...
                    {/* Badge */}
                    <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-md text-cokelat font-bold px-4 py-1.5 rounded-full z-10 shadow-md flex items-center gap-2 text-xs md:text-sm">
                  ...
                  <div className="p-6 md:p-8 relative flex-grow flex flex-col justify-between">
                    <div className="absolute -top-6 right-6 md:right-8 bg-krem w-12 h-12 rounded-full flex items-center justify-center border-4 border-white shadow-sm text-hijau">
                      <Heart size={20} fill="currentColor" />
                    </div>
                    <div>
                      <h3 className="font-playfair text-xl sm:text-2xl font-bold text-cokelat mb-3">{item.nama}</h3>
                      <div className="w-12 h-1 bg-emas mb-5 rounded-full"></div>
                      <p className="text-cokelat/70 text-sm md:text-base leading-relaxed mb-4">
                        {item.deskripsi}
                      </p>
                    </div>
                  </div>
