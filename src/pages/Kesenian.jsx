...
const Kesenian = () => {
  ...
  return (
    <div className="bg-krem min-h-screen">
      ...
      <section className="py-24 bg-krem">
        <div className="container mx-auto px-4 lg:px-8">
          <div className="flex flex-col lg:flex-row items-center gap-16 bg-white p-8 md:p-12 rounded-[40px] shadow-xl border border-emas/10">
            <motion.div 
            ...
            >
              <h2 className="text-hijau font-bold tracking-widest uppercase text-sm flex items-center gap-2">
                <Star size={16} /> Karya Mendunia
              </h2>
              <h3 className="font-playfair text-4xl md:text-5xl text-cokelat font-bold leading-tight">
      ...
      <section className="py-24 bg-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 lg:px-8 relative z-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
            <h2 className="text-emas font-bold tracking-widest uppercase text-sm mb-2">Koleksi Seni</h2>
            <h3 className="font-playfair text-4xl md:text-5xl font-bold text-cokelat mb-8">Eksplorasi Ragam Kesenian</h3>
            ...
            <AnimatePresence>
              {filteredData.map(item => (
                <motion.div
                  ...
                  className="bg-krem rounded-3xl overflow-hidden shadow-lg cursor-pointer group border border-emas/10 hover:shadow-2xl hover:border-emas/50 transition-all"
                >
                  <div className="h-72 relative overflow-hidden">
                    ...
                    <img 
                      src={`https://source.unsplash.com/600x400/?traditional,art,indonesia&sig=${item.id + 100}`}
                      alt={item.nama}
                    ...
                    <div className="absolute bottom-6 left-0 right-0 text-center z-20 transform translate-y-4 group-hover:translate-y-0 opacity-0 group-hover:opacity-100 transition-all duration-300">
                      <span className="inline-flex items-center gap-2 bg-emas text-cokelat px-6 py-2 rounded-full font-bold text-sm shadow-lg">
                        <Search size={16} /> Lihat Detail
                      </span>
                    </div>
                  </div>
                  <div className="p-8 text-center relative bg-white">
                    <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 w-12 h-12 bg-krem rounded-full flex items-center justify-center border-4 border-white z-20 text-emas">
                      <Music size={20} />
                    </div>
                    <h3 className="font-playfair text-2xl font-bold text-cokelat mb-3 mt-2">{item.nama}</h3>
        ...
        {selectedItem && (
          ...
          >
            <motion.div
              ...
              className="bg-white w-full max-w-4xl rounded-[40px] overflow-hidden shadow-2xl flex flex-col md:flex-row relative"
            >
              <button 
                onClick={() => setSelectedItem(null)}
              ...
              <div className="md:w-1/2 h-64 md:h-auto relative">
                <img 
                  src={`https://source.unsplash.com/800x800/?traditional,art,indonesia&sig=${selectedItem.id + 100}`}
              ...
              <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center bg-krem/30">
                <span className="inline-block bg-hijau/10 text-hijau font-bold tracking-wider uppercase text-xs px-4 py-2 rounded-full mb-4 self-start">
                  {selectedItem.kategori}
                </span>
                <h3 className="font-playfair text-4xl md:text-5xl font-bold text-cokelat mb-6">{selectedItem.nama}</h3>
                <div className="w-16 h-1 bg-emas mb-6"></div>
                <p className="text-cokelat/80 text-lg leading-relaxed mb-8 font-medium">
                  {selectedItem.deskripsi}
                </p>
                
                <div className="bg-white p-4 rounded-2xl border border-emas/20 flex items-center gap-4">
                  <div className="w-12 h-12 bg-emas/20 rounded-full flex items-center justify-center text-emas">
                    <Star size={24} />
                  </div>
                  <div>
                    <h5 className="font-bold text-cokelat">Warisan Budaya</h5>
                    <p className="text-xs text-cokelat/60">Dilestarikan turun temurun</p>
                  </div>
                </div>
              </div>
            </motion.div>
